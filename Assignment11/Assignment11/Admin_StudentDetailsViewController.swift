...
//          var Classlength: Int = ClassArray.count
            for i in 0..<ClassArray.count
            {
                ...
                print("st.Class = ",st.Class)
                if String(st.Class) == ClassArray[i]
                {
            ...
            let date = dateFormatter.date(from: st.Dob)
...
extension Admin_StudentDetailsViewController {
    ...
    @objc func OnAddStudentClicked()
    {
        ...
        if let st = student
        {
            ...
            let updatedStudent = Student(Spid: st.Spid, Name: Name, Gender: Gender, Dob: Dob, Class: Class, Password: st.Password, UserType: st.UserType)
            resetFields()
